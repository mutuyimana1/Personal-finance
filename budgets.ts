...
function getLatestTransactions(category: string): Transaction[] {
  return mockTransactions
    .filter(t => t.category === category && t.amount < 0)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const pieTotalLimit = document.getElementById('pieTotalLimit') as HTMLElement;
  const pieLegend = document.getElementById('pieLegend') as HTMLElement;
  
  // Budget List Elements
  const budgetCardsContainer = document.getElementById('budgetCardsContainer') as HTMLElement;
  
  // Modal Elements
  const budgetModal = document.getElementById('budgetModal') as HTMLElement;
  ...
  budgetForm.addEventListener('submit', (e) => {
    ...
    const maxSpend = parseFloat((document.getElementById('bSpend') as HTMLInputElement).value);
    const theme = (document.getElementById('bTheme') as HTMLSelectElement).value;
